...
int get_student(int fd, int id, student_t *s)
{
    ...
    // copy contents of file to s pointer and additional error handling
    ...
    {
        return SRCH_NOT_FOUND; // record is empty
    }
...
int add_student(int fd, int id, char *fname, char *lname, int gpa, bool compressed, int counter)
{
    ...
    if (read(fd, &existing_student, sizeof(student_t)) <= 0 || memcmp(&existing_student, &EMPTY_STUDENT_RECORD, sizeof(student_t)) == 0)
    {
        // actually create the new student struct
...
int del_student(int fd, int id)
{
    int status;
    int offset = id * sizeof(student_t);
    student_t to_be_deleted;
    status = get_student(fd, id, &to_be_deleted);
    ...
    // lseek to position of the record
    if (lseek(fd, offset, SEEK_SET) == -1)
    {
        printf("%s", M_ERR_DB_WRITE);
        return ERR_DB_FILE;
    ...
    // write empty record to delete the student
...
int count_db_records(int fd)
{
    ...
    while ((bytes_read = read(fd, &record, sizeof(student_t))) > 0)
    {
        ...
        // check if record is all zeros
        if (memcmp(&record, &EMPTY_STUDENT_RECORD, sizeof(student_t)) != 0)
        {
            count++;
        }
    }
